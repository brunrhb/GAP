/* ── AnimatedA ─────────────────────────────────────────────
   ...
   width   : auto  → viewBox dynamique, pousse les voisins
   pMax    : calculé si data-no-wrap, jamais de overflow
   ──────────────────────────────────────────────────────── */
...
class AnimatedA {
  ...
  /* ── Géométrie — seul p change, Y constants ── */
  ...
  /* ── Calcule p_max pour ne pas déborder de la ligne ── */
  ...
  measurePMax() {
    ...
    const rEdge = window.innerWidth - padR;
    ...
    while (node) {
      if (node.nodeType === 3 && node.textContent.trim()) {
        ...
        afterW += range.getBoundingClientRect().width;
      } else if (node.nodeType === 1) {
        afterW += node.getBoundingClientRect().width;
      }
...
/* ── AController ─────────────────────────────────────────
   ctrl.toggleAll()   pause / reprise
   ctrl.invert(n)     inverse le A n (0-indexé)
   ctrl.seekAll(0.5)  positionne tous à p=0.5 (debug)
   ──────────────────────────────────────────────────────── */
...
class AController {
  ...
  init(opts = {}) {
    document.fonts.ready.then(() => {
      // Premier RAF : DOM layout stable après insertion des SVG
      requestAnimationFrame(() => {
        ...
        // Second RAF : mesures précises après premier rendu
        requestAnimationFrame(() => {
          this.pool.forEach(i => {
            i.measurePMax();
            i.start(opts);
